...
server <- function(input, output, session) {
  values <- reactiveValues(n_fixed_areas = c(), n_bundles = 0)
        ...
        insertUI(
          ...
          ui = tags$div(
            ...
            fluidRow(
              column(3,
                     numericInput(
                       paste0("num_screen_", n_bundle, "_", values$n_fixed_areas[n_bundle], "_1"),
                       label = h5("Top left X"),
                       value = 0
                     )),
              column(3,
                     numericInput(
                       paste0("num_screen_", n_bundle, "_", values$n_fixed_areas[n_bundle], "_2"),
                       label = h5("Top left Y"),
                       value = 0
                     )),
              column(3,
                     numericInput(
                       paste0("num_screen_", n_bundle, "_", values$n_fixed_areas[n_bundle], "_3"),
                       label = h5("Bottom left X"),
                       value = 0
                     )),
              column(3,
                     numericInput(
                       paste0("num_screen_", n_bundle, "_", values$n_fixed_areas[n_bundle], "_4"),
            ...
            fluidRow(
              column(3,
                     numericInput(
                       paste0("num_image_", n_bundle, "_", values$n_fixed_areas[n_bundle], "_1"),
                       label = h5("Top left X"),
                       value = 0
                     )),
              column(3,
                     numericInput(
                       paste0("num_image_", n_bundle, "_", values$n_fixed_areas[n_bundle], "_2"),
                       label = h5("Top left Y"),
                       value = 0
                     )),
              column(3,
                     numericInput(
                       paste0("num_image_", n_bundle, "_", values$n_fixed_areas[n_bundle], "_3"),
                       label = h5("Bottom left X"),
                       value = 0
                     )),
              column(3,
                     numericInput(
                       paste0("num_image_", n_bundle, "_", values$n_fixed_areas[n_bundle], "_4"),
                       label = h5("Bottom left Y"),
                       value = 0
                     ))
            )
      ...
      for (bundle in 1:values$n_bundles)
      {
        ...
        if(values$n_fixed_areas[bundle] > 0)
        {
          ...
          for (fixed_area in 1:values$n_fixed_areas[bundle])
          {
            ...
            {
              argument_bundle_creation <- paste0("bundle_", bundle, "_fixed_area_", fixed_area,"_screen, ", "bundle_", bundle, "_fixed_area_", fixed_area,"_image")
            }
            else
            {
              argument_bundle_creation <- paste(argument_bundle_creation, paste0("bundle_", bundle, "_fixed_area_", fixed_area,"_screen, ", "bundle_", bundle, "_fixed_area_", fixed_area,"_image"), sep=",\n")
            }
            text <- paste0(text, "# Fixed area ", fixed_area, "\n")
            text <- paste0(paste0(text, "bundle_", bundle, "_fixed_area_", fixed_area,"_screen <- ", "c(", input[[paste0("num_screen_", bundle,"_", fixed_area,"_1")]], ", ", input[[paste0("num_screen_", bundle,"_", fixed_area,"_2")]], ", ", input[[paste0("num_screen_", bundle,"_", fixed_area,"_3")]], ", ", input[[paste0("num_screen_", bundle,"_", fixed_area,"_4")]], ")\n"))
          ...
          {
            function_to_print <- switch(strtoi(input[[paste0("select_", bundle)]]),
                                        {print_true()},
                                        {print_before_scroll()},
                                        {print_after_scroll()},
                                        {print_custom()})
            ...
            {
              argument_list_rules <- str_split(function_to_print, pattern = " ")[[1]][1]
            }
            else
            {
