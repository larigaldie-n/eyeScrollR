test_that("Properly modifies the rule flags", {
  ...
  expect_equal(check_rules_true(rules, data_line, flags, fixed_areas, scroll), c(TRUE, FALSE, TRUE))

  scroll <- 40
  expect_equal(check_rules_true(rules, data_line, flags, fixed_areas, scroll), c(FALSE, TRUE, TRUE))
})
